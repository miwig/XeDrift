#taken from https://github.com/nipy/dipy/blob/master/dipy/tracking/local/interpolation.pyx with minimal modifications
...
cpdef trilinear_interpolate4d(double[:, :, :, :] data, double[:] point,double[:] ds_inv,double[:] mins,
                              np.ndarray out=None):
    """Tri-linear interpolation along the last dimension of a 4d array
    ...
    cdef double point2[3]
    
    cdef int i
    for i in range(3):
        point2[i] = (point[i] - mins[i]) * ds_inv[i]
    
    #point = (point-mins)*ds_inv

    err = trilinear_interpolate4d_c(data, &point2[0], out)
